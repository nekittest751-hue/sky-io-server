  ...
  attachSocketHandlers(socket, {
    onMessage: (text) => {
      ...
      if (msg.type === 'join') {
        ...
        if (deviceRecord.modChatOnly && !wantsModChat) {
          wsSend(socket, {
            type: 'joinRejected',
            reason: DEVICE_LOCK_WARNING,
            trainingOnly: true
          });
          setTimeout(() => {
            try { socket.end(); } catch {}
        ...
        if (wantsModChat) {
          if (!consentAccepted) {
            wsSend(socket, {
              type: 'joinRejected',
              reason: 'Сначала нужно согласиться с предупреждением для «чат без ограничений».',
              trainingOnly: true
            });
            setTimeout(() => {
              try { socket.end(); } catch {}
