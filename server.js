...
const CURRENT_STANDARD_VERSION = '2.3.0-standard-panel';
const CURRENT_MOD_VERSION = '2.3.0-mod-panel';
const MAX_ROOM_PLAYERS = 10;
const ROUND_DURATION_OPTIONS = [120, 180, 300];
const DEFAULT_ROUND_DURATION = 180;
const VOTE_INTERMISSION_SEC = 30;
const DEFAULT_JUMP_POWER = 14.8;
...
function summarizeRoom(room) {
  ...
  const phase = room.roundPhase || 'active';
  const phaseTarget = phase === 'voting' ? (room.voteEndsAt || Date.now()) : (room.roundEndsAt || Date.now());
  return {
    room: room.name,
    mapKey: room.mapKey,
    ...
    roundEndsInSec: Math.max(0, Math.ceil(((room.roundEndsAt || Date.now()) - Date.now()) / 1000)),
    phase,
    phaseEndsInSec: Math.max(0, Math.ceil((phaseTarget - Date.now()) / 1000)),
    voteEndsInSec: Math.max(0, Math.ceil(((room.voteEndsAt || 0) - Date.now()) / 1000))
  };
}
...
function serializeRound(room, player = null) {
  ...
  return {
    ...
    phase: room.roundPhase || 'active',
    endsAt: room.roundEndsAt || (Date.now() + DEFAULT_ROUND_DURATION * 1000),
    voteEndsAt: room.voteEndsAt || 0,
    voteWindowSec: VOTE_INTERMISSION_SEC,
    mapKey: room.mapKey,
    mapName: room.map.name,
    mapVotes,
    durationVotes,
...
function startVotingPhase(room) {
  if (room.roundPhase === 'voting') return;
  room.roundPhase = 'voting';
  room.voteEndsAt = Date.now() + VOTE_INTERMISSION_SEC * 1000;
  room.bullets = [];
  for (const player of room.players.values()) {
    player.input.shoot = false;
    player.shootCooldown = Math.max(player.shootCooldown || 0, 6);
  }
  broadcastSystemToRoom(room, `Раунд закончился. ${VOTE_INTERMISSION_SEC} сек. на голосование за карту и длительность.`, STANDARD_CHAT_CHANNEL);
  broadcastSystemToRoom(room, `Раунд закончился. ${VOTE_INTERMISSION_SEC} сек. на голосование за карту и длительность.`, MOD_CHAT_CHANNEL);
...
function advanceRoomRound(room) {
  ...
  room.roundPhase = 'active';
  room.roundEndsAt = Date.now() + nextDurationSec * 1000;
  room.voteEndsAt = 0;
  room.mapVotes.clear();
...
function tickRoom(room) {
  ...
  const now = Date.now();
  if (room.roundPhase === 'voting') {
    if (now >= (room.voteEndsAt || 0)) advanceRoomRound(room);
  } else if (now >= (room.roundEndsAt || 0)) {
