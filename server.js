...
const ROUND_WIN_COINS = 30;
const MOBILE_JUMP_POWER = 14.8;

const SERVER_NAME = 'Sky .io Mobile Arena';
const MIN_CHAT_VERSION = '2.0.0';
const MIN_RECOMMENDED_CLIENT_VERSION = '2.5.0';
...
const SKINS = {
  classic: { key: 'classic', name: 'Classic', price: 0, bodyColor: '#60a5fa', accentColor: '#ffffff' },
  neon:    { key: 'neon',    name: 'Neon',    price: 40, bodyColor: '#22c55e', accentColor: '#d9f99d' },
  gold:    { key: 'gold',    name: 'Gold',    price: 90, bodyColor: '#f59e0b', accentColor: '#fef3c7' },
  shadow:  { key: 'shadow',  name: 'Shadow',  price: 140, bodyColor: '#6366f1', accentColor: '#e9d5ff' },
  coral:   { key: 'coral',   name: 'Coral',   price: 180, bodyColor: '#fb7185', accentColor: '#ffe4e6' },
  rainbow: { key: 'rainbow', name: 'Rainbow ★', price: 320, bodyColor: '#8b5cf6', accentColor: '#ffffff' }
};
...
function buildRoomPickups(map) {
  ...
  for (const spot of extraMedkits) {
    pickups.push({ id: makeId('pickup-'), type: 'medkit', x: spot.x, y: spot.y, r: 16 });
  }
  return pickups;
}
...
function beginVotePhase(room) {
  ...
  if (winners.length) {
    for (const winner of winners) {
      winner.coins = Number(winner.coins || 0) + ROUND_WIN_COINS;
      winner.wins = Number(winner.wins || 0) + 1;
      persistPlayerProgress(winner);
      sendProgress(room.sockets.get(winner.id), winner, `Победа в раунде: +${ROUND_WIN_COINS} монет`);
    }
    broadcastSystemAllChannels(room, `Раунд #${room.round.number} выигран. Награда победителям: +${ROUND_WIN_COINS} монет.`);
  } else {
    ...
    onMessage: (text) => {
      ...
      if (msg.type === 'join') {
        ...
        if (isOldClientVersion(player.clientVersion)) {
          sendSystemMessage(socket, `Сервер: у вас старая версия клиента (${player.clientVersion}). Рекомендуется обновиться до ${MIN_RECOMMENDED_CLIENT_VERSION}+.`);
          if (player.chat.canRead) {
