...
function censorText(text) {
  return text.replace(PROFANITY_RE, (m) => '*'.repeat(Math.max(3, m.length)));
}
...
function broadcastSystemToRoom(room, text) {
  const entry = {
    id: makeId('sys-'),
    from: 'Сервер',
    text,
  ...
  const safeClose = () => {
    ...
    if (room && playerId) {
      ...
      if (player && player.chat.canRead) {
        broadcastSystemToRoom(room, `${name} вышел из комнаты.`);
      }
  ...
  attachSocketHandlers(socket, {
    onMessage: (text) => {
      ...
      if (msg.type === 'join') {
        ...
        if (player.chat.canRead) {
          broadcastSystemToRoom(room, `${player.name} вошел в комнату.`);
        }
        return;
      }
