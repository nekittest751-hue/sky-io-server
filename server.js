...
function isLegacyClientVersion(version) {
  const normalized = normalizeClientVersion(version);
  if (!normalized) return true;
  if (!semverGte(normalized, '2.1.0')) return true;
  if (normalized.startsWith('2.0.')) return true;
  ...
  attachSocketHandlers(socket, {
    onMessage: (text) => {
      ...
      if (msg.type === 'join') {
        ...
        if (isLegacyClientVersion(player.clientVersion)) {
