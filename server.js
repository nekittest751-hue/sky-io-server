...
const server = http.createServer((req, res) => {
  if (req.url === '/rooms') {
    const data = Array.from(rooms.values())
      .sort((a, b) => a.baseName.localeCompare(b.baseName) || a.shardIndex - b.shardIndex)
      .map((room) => ({
        room: room.name,
    roomBase: room.baseName,
    roomShard: room.shardIndex,
    roomCapacity: room.capacity,
        baseRoom: room.baseName,
        shard: room.shardIndex,
        mapKey: room.mapKey,
...
function normalizeBaseRoomName(name) {
  const cleaned = String(name || 'arena').trim().replace(/\s+/g, ' ').slice(0, 20);
  return cleaned || 'arena';
}

function parseRequestedRoom(value) {
  const cleaned = normalizeBaseRoomName(value);
  const match = cleaned.match(/^(.*?)(?:\s*#\s*(\d+))?$/);
  const baseName = normalizeBaseRoomName(match && match[1] ? match[1] : cleaned);
  const preferredShard = match && match[2] ? Math.max(1, parseInt(match[2], 10) || 1) : null;
  return { baseName, preferredShard };
}

function makeRoomKey(baseName, mapKey, shardIndex) {
  return `${baseName}::${mapKey}::${shardIndex}`;
}

function makeRoomName(baseName, shardIndex) {
  return shardIndex > 1 ? `${baseName} #${shardIndex}` : baseName;
}

function createRoom(baseName, shardIndex = 1, requestedMapKey = 'arena') {
  const map = cloneMap(requestedMapKey);
  const room = {
    key: makeRoomKey(baseName, map.key, shardIndex),
    name: makeRoomName(baseName, shardIndex),
    baseName,
    shardIndex,
...
function getRoomsForBase(baseName, requestedMapKey = 'arena') {
  const normalizedBase = normalizeBaseRoomName(baseName);
  const normalizedMapKey = (MAPS[requestedMapKey] ? requestedMapKey : 'arena');
  return Array.from(rooms.values())
    .filter((room) => room.baseName === normalizedBase && room.mapKey === normalizedMapKey)
    .sort((a, b) => a.shardIndex - b.shardIndex);
}

function getJoinRoom(name, requestedMapKey) {
  const parsed = parseRequestedRoom(name);
  const mapKey = MAPS[requestedMapKey] ? requestedMapKey : 'arena';
  const sameBaseRooms = getRoomsForBase(parsed.baseName, mapKey);

  if (parsed.preferredShard) {
    const preferred = sameBaseRooms.find((room) => room.shardIndex === parsed.preferredShard);
    if (preferred && preferred.players.size < preferred.capacity) {
      return preferred;
    }
    if (!preferred) {
      return createRoom(parsed.baseName, parsed.preferredShard, mapKey);
    }
  }

  const available = sameBaseRooms.find((room) => room.players.size < room.capacity);
  if (available) return available;

  const nextShard = sameBaseRooms.length ? Math.max(...sameBaseRooms.map((room) => room.shardIndex)) + 1 : 1;
  return createRoom(parsed.baseName, nextShard, mapKey);
}
...
function handlePickup(room, p) {
  ...
  for (let i = room.pickups.length - 1; i >= 0; i--) {
    ...
    if (dx * dx + dy * dy < 42 * 42) {
      ...
      setTimeout(() => {
        const existingRoom = rooms.get(room.key);
...
function cleanupRoom(roomName) {
  ...
  if (room.players.size === 0 && room.sockets.size === 0) {
    ...
    rooms.delete(room.key || roomName);
  }
}
...
server.on('upgrade', (req, socket) => {
  ...
  attachSocketHandlers(socket, {
    onMessage: (text) => {
      ...
      if (msg.type === 'join') {
        room = getJoinRoom(msg.room, msg.mapKey);
        playerId = makeId('p-');
        ...
        wsSend(socket, { type: 'roomAssigned', room: room.name, baseRoom: room.baseName, shardIndex: room.shardIndex, roomCapacity: room.capacity, players: room.players.size, mapKey: room.mapKey });
        sendChatState(socket, player);
...
server.listen(PORT, () => {
  console.log(`${SERVER_NAME} listening on ws://localhost:${PORT} with auto rooms up to ${MAX_PLAYERS_PER_ROOM} players each`);
});