...
const CURRENT_STANDARD_VERSION = '2.1.0-standard-lowlatency';
const CURRENT_MOD_VERSION = '2.1.0-mod';
...
function getRecommendedVersionForChannel(channel) {
  return normalizeChatChannel(channel) === MOD_CHAT_CHANNEL
    ? CURRENT_MOD_VERSION
...
function isLegacyClientVersion(version, channel = STANDARD_CHAT_CHANNEL) {
  ...
  // Любая промежуточная/старая сборка на том же базовом semver считается устаревшей,
  // если не совпадает с текущей рекомендованной строкой версии для этого канала.
...
function buildLegacyVersionWarning(player) {
  ...
  return `Сервер: у вас старая или промежуточная версия клиента (${actual}). Рекомендуется обновиться до ${current}. На старых версиях могут быть лаги, проблемы чата и несовместимость с новыми функциями.`;
}
