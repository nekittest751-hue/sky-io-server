...
const SERVER_NAME = 'Sky .io Vote + Rooms Server';
...
const CURRENT_STANDARD_VERSION = '2.4.0-standard-panelvote';
const CURRENT_MOD_VERSION = '2.4.0-mod-panelvote';
const MAX_ROOM_PLAYERS = 10;
const ROUND_DURATION_OPTIONS = [120, 180, 300];
...
function summarizeRoom(room) {
  let modPlayers = 0;
  let standardPlayers = 0;
  for (const p of room.players.values()) {
    if (normalizeChatChannel(p.chat && p.chat.channel) === MOD_CHAT_CHANNEL) modPlayers += 1;
    else standardPlayers += 1;
  }
  return {
    room: room.name,
    mapKey: room.mapKey,
    ...
    roundDurationSec: room.roundDurationSec || DEFAULT_ROUND_DURATION,
    phase: room.roundPhase || 'battle',
    roundEndsInSec: Math.max(0, Math.ceil((((room.roundPhase === 'vote' ? room.voteEndsAt : room.battleEndsAt) || room.roundEndsAt || Date.now()) - Date.now()) / 1000))
  };
}
...
const server = http.createServer((req, res) => {
  ...
  if (req.url === '/rooms') {
    const data = Array.from(rooms.values()).map(summarizeRoom);
    res.writeHead(200, { 'Content-Type': 'application/json; charset=utf-8', ...corsHeaders });
    res.end(JSON.stringify({ server: SERVER_NAME, currentStandardVersion: CURRENT_STANDARD_VERSION, currentModVersion: CURRENT_MOD_VERSION, rooms: data }, null, 2));
    return;
  }

  if (req.url === '/mod-rooms') {
    const data = Array.from(rooms.values()).map(summarizeRoom).filter((room) => room.modPlayers > 0);
    res.writeHead(200, { 'Content-Type': 'application/json; charset=utf-8', ...corsHeaders });
    res.end(JSON.stringify({ server: SERVER_NAME, currentModVersion: CURRENT_MOD_VERSION, rooms: data }, null, 2));
    return;
  }
...
function getRecommendedVersionForChannel(channel) {
  return normalizeChatChannel(channel) === MOD_CHAT_CHANNEL ? CURRENT_MOD_VERSION : CURRENT_STANDARD_VERSION;
}

function isLegacyClientVersion(version, channel = STANDARD_CHAT_CHANNEL) {
  const normalized = normalizeClientVersion(version);
  const current = normalizeClientVersion(getRecommendedVersionForChannel(channel));
  if (!normalized) return true;
  const actualCore = extractVersionCore(normalized);
  const currentCore = extractVersionCore(current);
  if (!semverGte(actualCore, currentCore)) return true;
  if (actualCore === currentCore && normalized !== current) return true;
  return false;
}

function buildLegacyVersionWarning(player) {
  const current = getRecommendedVersionForChannel(player && player.chat ? player.chat.channel : STANDARD_CHAT_CHANNEL);
  const actual = String(player && player.clientVersion ? player.clientVersion : 'неизвестная версия');
  return `Сервер: у вас старая версия клиента (${actual}). Рекомендуется обновиться до ${current}. На старых версиях могут быть лаги, проблемы чата и несовместимость с новыми функциями.`;
}
...
function cloneRoomPickups(roomMap) {
  return roomMap.pickups.map((p) => ({ id: makeId('pickup-'), ...p, r: 16 }));
}

function sanitizeRoomName(name) {
  return String(name || 'arena').trim().replace(/\s+/g, ' ').slice(0, 20) || 'arena';
}

function truncateRoomWithSuffix(base, suffix) {
  const limit = Math.max(1, 20 - suffix.length);
  return `${base.slice(0, limit)}${suffix}`;
}

function isExplicitRoomShard(name) {
...
function createRoom(name, requestedMapKey = 'arena') {
  ...
  const room = {
    ...
    sockets: new Map(),
    bullets: [],
    pickups: cloneRoomPickups(map),
    chatHistory: [],
    lastStateBroadcastAt: 0,
    fastStateTimer: null,
    roundNumber: 1,
    roundDurationSec: DEFAULT_ROUND_DURATION,
    roundPhase: 'battle',
    battleEndsAt: Date.now() + DEFAULT_ROUND_DURATION * 1000,
    voteEndsAt: 0,
    roundEndsAt: Date.now() + DEFAULT_ROUND_DURATION * 1000,
    mapVotes: new Map(),
    durationVotes: new Map()
  };
  rooms.set(name, room);
  return room;
}
...
function getRoom(name, requestedMapKey) {
  const baseName = sanitizeRoomName(name);
  if (isExplicitRoomShard(baseName)) return rooms.get(baseName) || createRoom(baseName, requestedMapKey);
  const primary = rooms.get(baseName);
  if (!primary) return createRoom(baseName, requestedMapKey);
  if (primary.players.size < MAX_ROOM_PLAYERS) return primary;
  let shard = 2;
  while (shard < 999) {
    const roomName = truncateRoomWithSuffix(baseName, ` #${shard}`);
    const existing = rooms.get(roomName);
    if (!existing) return createRoom(roomName, requestedMapKey || primary.mapKey);
    if (existing.players.size < MAX_ROOM_PLAYERS) return existing;
    shard += 1;
  }
  return primary;
}
...
function pickVoteWinner(counts, fallbackValue, allowedValues = null) {
  ...
  for (const [value, count] of Object.entries(counts || {})) {
    ...
    if (count === bestCount && String(value) === String(fallbackValue)) bestValue = value;
  }
  return bestValue;
}

function serializeRound(room, player = null) {
  const mapVotes = tallyVotes(Array.from(room.mapVotes.values()));
  const durationVotes = tallyVotes(Array.from(room.durationVotes.values()).map((v) => String(v)));
  const phase = room.roundPhase || 'battle';
  const battleEndsAt = room.battleEndsAt || room.roundEndsAt || (Date.now() + DEFAULT_ROUND_DURATION * 1000);
  const voteEndsAt = room.voteEndsAt || 0;
  return {
    roundNumber: room.roundNumber || 1,
    durationSec: room.roundDurationSec || DEFAULT_ROUND_DURATION,
    phase,
    endsAt: phase === 'vote' ? (voteEndsAt || Date.now() + VOTE_DURATION_SEC * 1000) : battleEndsAt,
    battleEndsAt,
    voteEndsAt,
    voteWindowSec: VOTE_DURATION_SEC,
    mapKey: room.mapKey,
    mapName: room.map.name,
    mapVotes,
    durationVotes,
    durationOptions: ROUND_DURATION_OPTIONS,
    myVote: player ? {
      mapKey: room.mapVotes.get(player.id) || room.mapKey,
      durationSec: room.durationVotes.get(player.id) || room.roundDurationSec
    } : null
  };
}

function sendRoundInfo(socket, room, player) {
  try { wsSend(socket, { type: 'roundInfo', round: serializeRound(room, player) }); } catch {}
}

function broadcastRoundInfo(room) {
  for (const [id, socket] of room.sockets.entries()) {
    const player = room.players.get(id);
    if (!player || socket.destroyed) continue;
...
function startVotePhase(room) {
  if (!room || room.roundPhase === 'vote') return;
  room.roundPhase = 'vote';
  room.voteEndsAt = Date.now() + VOTE_DURATION_SEC * 1000;
  room.roundEndsAt = room.voteEndsAt;
  broadcastSystemToRoom(room, `Раунд завершен. 30 сек на голосование за карту и длительность.`, STANDARD_CHAT_CHANNEL);
  broadcastSystemToRoom(room, `Раунд завершен. 30 сек на голосование за карту и длительность.`, MOD_CHAT_CHANNEL);
  broadcastRoundInfo(room);
  broadcastState(room);
}

function advanceRoomRound(room) {
  const mapVotes = tallyVotes(Array.from(room.mapVotes.values()));
  const durationVotes = tallyVotes(Array.from(room.durationVotes.values()).map((v) => String(v)));
  const nextMapKey = pickVoteWinner(mapVotes, room.mapKey, Object.keys(MAPS));
  const nextDurationRaw = pickVoteWinner(durationVotes, String(room.roundDurationSec), ROUND_DURATION_OPTIONS.map((v) => String(v)));
  const nextDurationSec = ROUND_DURATION_OPTIONS.includes(Number(nextDurationRaw)) ? Number(nextDurationRaw) : room.roundDurationSec;
  room.roundNumber = (room.roundNumber || 1) + 1;
  room.roundDurationSec = nextDurationSec;
  room.roundPhase = 'battle';
  room.battleEndsAt = Date.now() + nextDurationSec * 1000;
  room.voteEndsAt = 0;
  room.roundEndsAt = room.battleEndsAt;
  room.mapVotes.clear();
  room.durationVotes.clear();
  room.bullets = [];
  if (nextMapKey !== room.mapKey) {
    const nextMap = cloneMap(nextMapKey);
    room.mapKey = nextMap.key;
    room.map = nextMap;
    room.world = { ...nextMap.world };
    room.pickups = cloneRoomPickups(nextMap);
  } else {
    room.pickups = cloneRoomPickups(room.map);
  }
  for (const player of room.players.values()) resetPlayerForNewRound(room, player);
  broadcastSystemToRoom(room, `Новый раунд #${room.roundNumber}. Карта: ${room.map.name}. Длительность: ${room.roundDurationSec} сек.`, STANDARD_CHAT_CHANNEL);
  broadcastSystemToRoom(room, `Новый раунд #${room.roundNumber}. Карта: ${room.map.name}. Длительность: ${room.roundDurationSec} сек.`, MOD_CHAT_CHANNEL);
  broadcastRoundInfo(room);
  broadcastState(room);
}
...
function serializeState(room) {
  ...
  return {
    ...
    world: room.world,
    platforms: room.map.platforms,
    round: serializeRound(room, null),
    players,
...
function tickRoom(room) {
  ...
  updateBullets(room);
  if ((room.roundPhase || 'battle') === 'battle' && Date.now() >= (room.battleEndsAt || room.roundEndsAt || 0)) startVotePhase(room);
  else if ((room.roundPhase || 'battle') === 'vote' && Date.now() >= (room.voteEndsAt || room.roundEndsAt || 0)) advanceRoomRound(room);
}
...
server.on('upgrade', (req, socket) => {
  ...
  const safeClose = () => {
    ...
    if (room && playerId) {
      ...
      room.mapVotes.delete(playerId);
      room.durationVotes.delete(playerId);
  ...
  attachSocketHandlers(socket, {
    onMessage: (text) => {
      ...
      if (msg.type === 'join') {
        ...
        sendSystemMessage(socket, `Сервер: ваша версия клиента — ${player.clientVersion || 'неизвестно'}.`);
        if (isLegacyClientVersion(player.clientVersion, player.chat.channel)) {
          sendSystemMessage(socket, buildLegacyVersionWarning(player));
        }
        sendRoundInfo(socket, room, player);

        if (player.chat.canRead) {
          if (normalizeChatChannel(player.chat.channel) === MOD_CHAT_CHANNEL) {
            broadcastSystemToRoom(room, `${player.name} вошел в комнату канала «${MOD_CHAT_LABEL}».`, MOD_CHAT_CHANNEL);
          ...
          if (isLegacyClientVersion(player.clientVersion, player.chat.channel)) {
            broadcastSystemToRoom(room, `${player.name} использует старую версию клиента (${player.clientVersion || 'неизвестно'}).`, player.chat.channel);
          }
        }
        return;
      }
      ...
      if (msg.type === 'voteRoundSettings') {
        if ((room.roundPhase || 'battle') !== 'vote') {
          sendSystemMessage(socket, 'Голосование откроется после окончания текущего раунда.');
          return;
        }
        const nextMapKey = typeof msg.mapKey === 'string' && MAPS[msg.mapKey] ? msg.mapKey : room.mapKey;
        const nextDurationSec = ROUND_DURATION_OPTIONS.includes(Number(msg.durationSec)) ? Number(msg.durationSec) : room.roundDurationSec;
        room.mapVotes.set(player.id, nextMapKey);
        room.durationVotes.set(player.id, nextDurationSec);
        sendSystemMessage(socket, `Голос принят: карта ${MAPS[nextMapKey].name}, длительность ${nextDurationSec} сек.`);
        broadcastRoundInfo(room);
        return;
      }
